<template>
	...
	>
					...
					>
						<AFormItem>
							<ASpace>
								<AButton
									@click="onReset"
									:disabled="state.loading || selectState.open"
									type="dashed"
									html-type="button"
									>重置
								</AButton>
			...
			>
				<template #bodyCell="{ column, record, index }">
					<template v-if="column.dataIndex === 'name'">
						<a
							v-if="buttonShow['visiDoctor_2'] && record?.state === 4"
					...
					<template v-if="column.dataIndex === 'action'">
						<ASpace>
							...
							<AButton
								@click="detailShowNew(record, index)"
								v-auth="'doctorInfo_2'"
								type="link"
								>详情</AButton
							>
							<AButton
								@click="editShow(record, index)"
								v-auth="'editDoctor_2'"
								type="link"
								>编辑</AButton
							>
							...
							<AButton
								@click="businessShow(record, index)"
								v-auth="'doctorBus_2'"
								type="link"
								>业务管理</AButton
							>
							<pharmacyManagement
								...
								v-auth="'doctorBus_2'"
							>
								<AButton type="link">药房管理</AButton>
							</pharmacyManagement>
							<AButton
								@click="assistantShow(record, index)"
								v-auth="'doctorAss_2'"
								type="link"
								>医助管理</AButton
							>
		...
		<DoctorDetailDrawer
			ref="doctorDetailDrawerRef"
			:isAuth="buttonShow.doctorAuth_2"
			@refresh="getData"
		/>
		...
		<template v-if="qrcodeState.visible">
			<DoctorQRCode
				:fatherData="qrcodeState"
				@coverClose="qrcodeCancel"
			/>
		</template>
		...
		<template v-if="businessState.visible">
			<DoctorBusiness
				:fatherData="businessState"
				@coverClose="businessCancel"
			/>
		</template>
		...
		<template v-if="assistantState.visible">
			<DoctorAssistant
				:fatherData="assistantState"
				@coverClose="assistantCancel"
			/>
		</template>

		<!-- 批量更改药房 -->
		<MultiBindPharmacy
			ref="multiBindPharmacyRef"
			@finished="onMultiBindPharmacyClose"
		/>
	</PageView>
</template>
...
import { useDoctorAction, useDoctorAssistant, useDoctorBusiness, useDoctorDetail, useDoctorQRCode, useDoctorRemove } from './hooks';
...
function getData() {
	...
	httpPost(`/nld/doctor/queryDoctorList`, params)
		.then((res) => {
...
function exportExcel() {
	...
	const params = getQueryParams();
	httpPost(`/nld/doctor/exportDoctor`, params, { responseType: 'blob' })
		.then((res) => {
...
function onSelectAllData(selected: boolean) {
	...
	if (selected) {
		updateSelectKeys(
			selected,
			state.dataSource.filter((item) => item.state === 4).map((item) => item.id)
		);
	}
}
